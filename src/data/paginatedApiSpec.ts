export const paginatedApiSpec = {
  ...
  tags: [
    {
      name: "پیام‌های دریافتی و انتقال ترافیک (Traffic Relay)",
      description: `از این متد برای دریافت پیامک‌های ورودی خط اختصاصی در وب‌سرویس قاصدک استفاده می‌شود. این متد امکان واکشی پیامک‌ها را فراهم می‌کند و پیام‌هایی که در پاسخ دریافت می‌شوند با وضعیت خوانده‌شده (isRead = 1) ثبت خواهند شد. کاربران می‌توانند از طریق پنل کاربری قاصدک یک URL برای انتقال ترافیک پیامک‌های دریافتی تعریف کنند تا پیام‌ها به‌صورت لحظه‌ای به سیستم آن‌ها ارسال شود. لاگ فراخوانی‌های انجام‌شده به این URL در پنل کاربری قابل مشاهده است.
      در صورتی که پیامک‌ها به هر دلیل از طریق URL دریافت نشوند، وضعیت آن‌ها خوانده‌نشده (isRead = 0) باقی می‌ماند و امکان دریافت آن‌ها از طریق این متد وجود دارد.`,
    },
  ],
  paths: {
